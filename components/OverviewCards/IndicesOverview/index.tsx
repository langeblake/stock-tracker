...
const fetchIndexData = async (
  ...
  const formattedSevenDaysBeforeDate = sevenDaysBeforeDate
    .toISOString()
    .split("T")[0];
      ...
      <div>
        {indicesData.slice(0, 2).map((indexData, idx) =>
          ...
          indexData ? (
            <div key={idx}>
              ...
              <div className="flex flex-col gap-10">
                ...
                <div className="flex justify-between gap-4">
                  ...
                  <div
                    className={`font-light ${indexData.results[0]?.c > indexData.results[1]?.c ? "text-green-500" : "text-red-500"}`}
                  >
                    {formatNumber(
                      (indexData.results[0]?.c ?? 0) -
                        (indexData.results[1]?.c ?? 0)
                    )}
                  </div>
                  <div>
                    $
                    {indexData.results[0]?.c?.toLocaleString(undefined, {
                ...
                <div className="flex justify-between">
                  ...
                  <div>
                    $
                    {indexData.results[0]?.o?.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    }) ?? "N/A"}
                  </div>
                ...
                <div className="flex justify-between">
                  ...
                  <div>
                    $
                    {indexData.results[0]?.h?.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    }) ?? "N/A"}
                  </div>
                ...
                <div className="flex justify-between">
                  ...
                  <div>
                    $
                    {indexData.results[0]?.l?.toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    }) ?? "N/A"}
                  </div>
            ...
            <div
              key={idx}
              className="shadow-lg rounded-lg p-4 border dark:border-zinc-700 dark:bg-zinc-900"
            >
