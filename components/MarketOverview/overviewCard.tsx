      ...
      <div className="border flex w-full flex-col gap-4 rounded-lg p-4 justify-around dark:border-zinc-700 dark:bg-zinc-900 bg-white dark:text-white">
        {isLoading ? (
          ...
          data.map((item, index) => {
            ...
            return (
              <div key={index} onClick={() => handleClick(item.ticker)} className="flex py-4 px-2 flex-row justify-between dark:hover:cursor-pointer dark:hover:bg-zinc-800 hover:bg-zinc-100 rounded-md">
